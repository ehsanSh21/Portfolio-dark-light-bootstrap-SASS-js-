...
/*********make modal dynamic********* */
const myModals = document.querySelectorAll('[data-bs-toggle="modal"]');
myModals.forEach(
    myModal =>
      myModal.addEventListener('click', (e) =>{
        if(myModal.classList.contains("weblogPic")){
            let myImgAddress = (myModal.src);
        
        const mymodalImage = document.querySelector('.myModalImg');
        mymodalImage.src = myImgAddress;
       
        

        } else {
            
            let myImgAddress = myModal.parentElement.parentElement.parentElement.parentElement.children[0].childNodes[1].src;
            console.log(myImgAddress)
            const mymodalImage = document.querySelector('.myModalImg');
            mymodalImage.src = myImgAddress;
        }
    ...
    function scrollFunc(e){
        const myAttLink = this.getAttribute('myAtt');
        const newstr = myAttLink.replace('#', '');
        console.log(newstr);
        if (newstr == ""){
            scrollTo(0,0,{behavior: 'smooth'});
        } else {
            document.getElementById(`${newstr}`).scrollIntoView({behavior: 'smooth'});
        };
    }

    navLinks.forEach(navLink => navLink.addEventListener('click',scrollFunc));

     /***********add class active to navlinks */
     const navbarLinks= document.querySelectorAll('a.nav-link');

     function addActive(e){
        navbarLinks.forEach(navLink => navLink.classList.remove("navActive"));
        this.classList.add("navActive");

     }
     navbarLinks.forEach(navLink => navLink.addEventListener('click',addActive));
     ...
     modeBtns.forEach(modeBtn => modeBtn.addEventListener('click', changeMode));
     modeBtns.forEach(modeBtn => modeBtn.addEventListener('click', changedirection));
     modeBtns[1].innerHTML = modeBtns[0].innerHTML;

   // modeBtns.forEach(modeBtn => modeBtn.classList.remove("forLight"));


   
      /**change dir(svg come from outside) */

      function backing(){
        console.log("dsfsdf");
        modeBtns.forEach(modeBtn => modeBtn.firstChild.style.transform="translateX(0px)");
     } 


     function changedirection(e){
        this.firstChild.style.transform="translateX(35px)";
        setTimeout(backing,130);
     }

   /*close navbar when clickig outside*/
